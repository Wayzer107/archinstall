...
class Installer:
	...
	def mount_ordered_layout(self, layouts: Dict[str, Any]) -> None:
		...
		for partition in [entry for entry in list_part if entry.get('encrypted', False)]:
			...
			if (self._has_root(partition)
			    and partition.get('generate-encryption-key-file', False) is False
			    and storage['arguments'].get('HSM')):
	...
	def minimal_installation(self, testing=False, multilib=False) -> bool:
		...
		for partition in self.partitions:
			...
			elif self.detect_encryption(partition) and 'encrypt' not in self.HOOKS:
